...
const handler = function (argv, message, acceptableFlags) {
  ...
  try {
    if (argv.json === true && acceptableFlags.json === true) {
      const jsonExport = JSON.stringify(message.json, {}, 2)
      return output(jsonExport, 'json', message.status)
    }
    ...
    if (argv.plain === true && acceptableFlags.plain === true) {
      return output(message.textPlain, 'human', 'none')
    }
    ...
    return output(message.error, 'human', 'error')
  }
}
