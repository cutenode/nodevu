...
exports.handler = function (argv) {
  request(api.latest, function (error, response, body) {
    ...
    for (var v in versions) {
      ...
      const message = {
        'text': `The latest release in the Node.js ${chalk.blue('v' + v)} release line is ${chalk.green(versions[v])}`, // the string to log if no specical flag is passed
        'textPlain': `The latest release in the Node.js ${v} release line is ${versions[v]}`, // the string to log if the --plain flag is passed
        'json': {
          'releaseLine': v,
          'release': versions[v]
        },
