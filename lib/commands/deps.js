...
exports.handler = function (argv) {
  request(api.data, function (error, response, body) {
    ...
    // TODO: this currently errors if process.version is used – not okay 😬
...
function check (obj, valueToCompare) {
  const checkIfThereIsALetterV = valueToCompare.indexOf('v')
  let validated

  if (checkIfThereIsALetterV === -1) {
    validated = `v${valueToCompare}`
  } else {
    validated = valueToCompare
  }

  for (var entry in obj) {
    if (obj[entry].version === validated) {
      return entry
    }
  }
}
