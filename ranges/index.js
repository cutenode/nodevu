...
async function generateRanges(filter) {
	...
	if (typeof filter === 'string') {
		...
		if (!aliases.includes(filter)) {
			throw new Error(
				`Unknown value passed as a filter. The passed value: '${filter}'.`,
			);
		}
		...
		if (!filter.every((alias) => aliases.includes(alias))) {
			throw new Error(
				`At least one of the values passed as a filter is unknown. The passed value(s): '${filter.join(
					"', '",
				)}'`,
			);
		}
		...
		throw new Error(
			`Unknown value passed as a filter. The passed value: '${filter}', with a type of '${typeof filter}'. Make sure you are passing a string (or an Array of strings) with valid value(s).`,
		);
	} else {
		localFilter = 'all';
	}
	...
	for (const line in data) {
		for (const key in data[line].releases) {
			...
			if (
				...
				filter === 'lts/active' ||
				filter === 'all' ||
				filter.includes('lts/latest') ||
				filter.includes('lts/active') ||
				filter.includes('all')
			) {
				if (
			...
			if (
				filter === 'lts/maintenance' ||
				filter === 'lts/active' ||
				filter === 'all' ||
				filter.includes('lts/maintenance') ||
				filter.includes('lts/active') ||
				filter.includes('all')
			) {
				if (
