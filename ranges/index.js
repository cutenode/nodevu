...
async function generateRanges (filter) {
	...
	if (typeof localFilter === 'string') {
		...
		if (!aliases.includes(localFilter)) {
			throw new Error(`Unknown value passed as a filter. The passed value: '${localFilter}'.`)
		}
	} else if (Array.isArray(localFilter) === true) {
		// check to make sure that the filter is valid. if it's not, yeet.
		if (!localFilter.every(alias => aliases.includes(alias))) {
			throw new Error(`At least one of the values passed as a filter is unknown. The passed value(s): '${localFilter.join('\', \'')}'`)
		}
	} else if (localFilter) {
		throw new Error(`Unknown value passed as a filter. The passed value: '${localFilter}', with a type of '${typeof localFilter}'. Make sure you are passing a string (or an Array of strings) with valid value(s).`)
	} else {
		localFilter = 'all'
	}
	...
	for (const line in data) {
		for (const key in data[line].releases) {
			...
			// define 'current' data
			if (localFilter === 'current' || localFilter === 'all' || localFilter.includes('current') || localFilter.includes('all')) {
			...
			if (localFilter === 'lts/latest' || localFilter === 'lts/active' || localFilter === 'all' || localFilter.includes('lts/latest') || localFilter.includes('lts/active') || localFilter.includes('all')) {
			...
			if (localFilter === 'lts/maintenance' || localFilter === 'lts/active' || localFilter === 'all' || localFilter.includes('lts/maintenance') || localFilter.includes('lts/active') || localFilter.includes('all')) {
			...
			// define 'eol' data
			if (localFilter === 'eol' || localFilter === 'all' || localFilter.includes('eol') || localFilter.includes('all')) {
	...
	if (localFilter === 'lts/active' || localFilter === 'all' || localFilter.includes('lts/active') || localFilter.includes('all')) {
